sap.ui.define([
		...
		return BaseController.extend("opensap.manageproducts.controller.Worklist", {
			...
			onDel : function (oEvent) {
				var oLItem = oEvent.getSource().getParent();
				var oLItemContext = oLItem.getBindingContext();
				this._deleteFromModel(oLItemContext.getPath(), oLItemContext.getProperty("Name"));
			},
			
			_deleteFromModel : function (sPath, sProductName) {
				var func = this._getDeleteSuccessFunction(sProductName);
				this.getModel().remove(sPath, {
					success : func.bind(this),
					error : this._deleteError.bind(this)
				});
			},

			_getDeleteSuccessFunction : function(sProductName){
				return function(oData, response){
					var msg = sProductName + " deleted";
